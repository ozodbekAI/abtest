...
export function ABTestsPage() {
  ...
  const [activeId, setActiveId] = useState<number | null>(() => Number(localStorage.getItem(ACTIVE_STORE_KEY)) || null);
  ...
  const loadConnections = async () => {
    try {
      ...
      if (selected) { setActiveId(selected.id); localStorage.setItem(ACTIVE_STORE_KEY, String(selected.id)); }
    } catch (error: any) {
  ...
  const setStore = (id: number) => { setActiveId(id); localStorage.setItem(ACTIVE_STORE_KEY, String(id)); };
...
function CreateTestModal({ connection, onClose, onCreated }: { connection: WBConnection | null; onClose: () => void; onCreated: (test: ABTest) => void }) {
  ...
  const canProceed = step === 1 ? Boolean(card) : step === 2 ? testImages.length >= 2 : fundingReady;
  ...
  const goNext = () => {
    ...
    if (step === 2 && testImages.length < 2) { toast.error('Добавьте минимум два изображения для сравнения'); return; }
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (testImages.length < 2) { toast.error('Добавьте минимум два изображения для сравнения'); setStep(2); return; }
